    ...
    public function handle()
    {
        ...
        if(isset($enable_email) && $enable_email == 'true')
        {
            ...
            if(isset($template['status']) && $template['status'] == 1)
            {
            	...
            	$var = array('{first_name}', '{last_name}', '{business_name}');
                $val = array($this->data['first_name'], $this->data['last_name'], $this->custom->getSetting('BUSINESS_NAME'));
                
                $template_cotent = str_replace($var, $val, $template['content']);
                
                
                Mail::to($this->data['email'])->send(new EmailCustomer($template_cotent));
